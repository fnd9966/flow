...
class Canvas_fnd extends React.Component {
	...
	handleOk = () => {
		...
		for (let i = 0,nLen = this.flowNode.length; i < nLen; i++) {
			...
			if(!!point) {
				...
				nodeData[key].left = 0;
				nodeData[key].top = 0;
				this.defaultNodeWH(this.flowNode, this.flowRoute);
			}
	...
	// 		,height = 26
	// 		,i = 0;
	// 	let canvasCenter = document.getElementById('canvas_arrow').offsetWidth / 2;

	// 	for (let i = 0, nLen = node.length; i < nLen; i++) {
	// 		let wh = {width: width, height: height};
	// 		if(node[i].index_id && node[i].index_id == 1) {
	// 			// console.log(node[i].node_id);
	// 			// node[i].point = Object.assign({}, {left: width / 2 - canvasCenter, top: 10}, wh)
	// 			node[i].point = {step: 1};
	// 			let arr = [];
	// 			let sort = {};
	// 			for (let j = 0, jLen = route.length; j < jLen; j++) {
	// 				// if(route[j].from_id == node[i].node_id || route[j].to_id == node[i].node_id) {
						
	// 				// }
	// 				// let arr = [];
	// 				if(route[j].from_id == node[i].node_id) {
	// 					arr.push(route[j].to_id)
	// 				}
	// 				if(route[j].to_id == node[i].node_id) {
	// 					arr.push(route[j].from_id)
	// 				}
	// 			}
	// 			sort.
	// 		}
