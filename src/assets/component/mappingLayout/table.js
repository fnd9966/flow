    ...
    operation = (e,key) => {
        ...
        switch (key) 
        {
            ...
            case 'addRow': 
                ...
                let r_i = s.x0, r_j = s.y0;
                // for (let rInd = 0; rInd < colLen; rInd++) {
                //     newRow.push({text: '', row: 1, col: 1});
                // }
                for (let j = 0; j < t[r_i].length; j++) {
                    if(t[r_i][j].p || t[r_i][j].row > 1) {
                        // console.log(1);
                        newRow[j] = ({text:'', row: 1,col: 1,p:{i: s.x0, j: s.y0}});
                        var adH = function() {
                            if(!t[r_i][j].p) {
                                return t[r_i][j].row += 1;
                            }
                            return t[r_i - 1][j].row += 1;
                        }
                        
                        // t[r_i][j].row += 1;
                        
                        for(let rInd =  0; rInd < colLen - t[r_i][j].col;rInd++) {
                            // console.log(colLen - t[r_i][j].row + 1);
                    ...
                    return <tr style = {{height: this.data.lineHeight}}>
                        {
                            tr.map((td,j) => {
                                ...
                                let tdH = ((this.state.select || this.state.isSelect) && this.inner(i,j)) ? 'highLight' : '',
                                    ...
                                    tdT = !!td.th ? 'isTh' : '';
                                _col++;
                                return <td style = {{textAlign: !td.textAlign ? 'left' : td.textAlign}} className = {tdC + ' ' + tdT + ' ' + tdH } rowSpan = {td.row} colSpan = {td.col} onClick = {e => this.focus(e,i,cInd)} onMouseDown = {e => this.startSelect(e,i,j)} onContextMenu = {e => this.openMenu(e,i,j)} onMouseMove = {e => this.selection(e,i,j)}>
