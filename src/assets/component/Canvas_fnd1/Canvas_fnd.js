...
import $ from 'jquery';
import ConfigModal from './form_fnd.js';
...
class Canvas_fnd extends React.Component {
	...
	removeFlow = false; //是否移除当前流程图，只有当保存的时候，才为true，才能直接移除，只有有编辑但是没有保存的情况，都会提醒
	ctx 
	target = false
	ele
	allFlowData = {}
	...
	componentDidMount() {
		 ...
		 * 双击节点弹出配置框
		 * @author fnd
		 * @param  {[type]} ".ico"     [description]
		 * @param  {[type]} "dblclick" [description]
		 * @param  {[type]} (e         [description]
		 * @return {[type]}            [description]
		 */
		this.workArea.delegate(".ico","dblclick",(e) => {
			let id = $(e.target).parents('.GooFlow_item').attr('id');
			this.focusId = id; //当前点击的节点id
			let focusNode = {};
			let nodeData = {};
			this.removeFlow = false;
			this.setState({
			...
			this.resetLines();
			//判断如果是已存在的flownode,需要处理所需要的nodeinfo
			if(this.flowNode && !this.oriFlowData) {
				for (let i = 0, nLen = this.flowNode.length; i < nLen; i++) {
					...
					let point = JSON.parse(this.flowNode[i].point); //解析node中的point字段，拿到我们需要的字段
					if(point) {
						let attr, api;
						// let attr = point.nodeInfo.attribute;
						// let api = point.nodeInfo.api;
						if(point.nodeInfo && point.nodeInfo.attribute) {
							attr = point.nodeInfo.attribute
							// attr = eval(attr);
	...
	handleSaveData = (data) => {
		...
		// console.log(this.nodeData[this.focusId]);
		this.nodeData[this.focusId].nodeInfo = data;
	...
	setName = (id,name,type) => {
		...
		if(type == 'node') {
			...
			this.$nodeDom[id].find('td:eq(0)').text(name);
	...
	addNode = (id,json) => {
		...
		// if(json.type.indexOf('node') < 0) {
			...
			// this.$nodeDom[id] = $("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_move'></i></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'>x</div></div></div>");
			this.$nodeDom[id] = $("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr class = 'ico' style='text-align:center'><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'>x</div></div></div>");
		// }else {
		// 	json.width = 26;
		// 	json.height = 26;
	...
	handleOk = () => {
		...
		this.editable = true;
		this.oriFlowData = false;//节点配置没有点保存的时候，即第一次双击打开
		let data = {};
		let max;
		//处理node数据
		let nodeData = {};
		for (let i = 0,nLen = this.flowNode.length; i < nLen; i++) {
			...
			let point = this.flowNode[i].point;
			if(!!point) {
				point = JSON.parse(this.flowNode[i].point);
				nodeData[key].width = point.width;
				nodeData[key].height = point.height;
				nodeData[key].left = point.left;
				nodeData[key].top = point.top;
				// nodeData[key].nodeInfo = point.nodeInfo;
				// this.max = point.max;
				max = point.max;
			}else {
				let n = this.defaultNodeWH(this.flowNode, this.flowRoute);
				// console.log(n);
				point = n[i].point;
				nodeData[key].width = point.width;
				nodeData[key].height = point.height;
	 ...
	 * 判断数组中的对象是否含有某个属性
	 */
	fieldInArrObj = (arr, field) => {
		if(arr.length !== 0) {
			for (let i = 0, aLen = arr.length; i < aLen; i++) {
				for (let key in arr[i]) {
					for (let j = 0, fLen = field.length; j < fLen; j++) {
						if(key == field[j]) {
							delete arr[i][key]
						}
					}
		...
		if(this.state.editFlow) {
			...
			for (let key in this.nodeData) {
				...
				n.point = {
					...
					max:this.max,
					nodeInfo:this.nodeData[key].nodeInfo
				};
				if (this.nodeData[key].nodeInfo) {
					for (let k in this.nodeData[key].nodeInfo) {
						if(k == 'attribute' && this.nodeData[key].nodeInfo[k]) {
							let a = this.nodeData[key].nodeInfo[k];
							this.fieldInArrObj(a, ['keys']);
						}
						if(k == 'api' && this.nodeData[key].nodeInfo[k]) {
							let b = this.nodeData[key].nodeInfo[k];
							this.fieldInArrObj(b, ['key', 'editable']);
						}
			...
			if(nodes.length > 0 && routes.length > 0) {
				...
				if(!!_type && _type == 'edit') {
					if(!this.isEqual) {
						this.postFlowType(this.flow_id, this.flowType);
				...
				this.postFlowNode(n, this.flow_id);
				this.postFlowRoute(r, this.flow_id);
	...
	dataProcess = (data, flow_id, type) => {
		if(type == 'node') {
			for (let i = 0, nLen = data.length; i < nLen; i++) {
				if(typeof(data[i].node_id) !== "number" && data[i].node_id.indexOf('_') !== -1) {
			...
			for (let j = 0, lLen = data.length; j < lLen; j++) {
				if(typeof(data[j].from_id) !== "number" && data[j].from_id.indexOf('_') !== -1) {
					data[j].from_id = data[j].from_id.replace(/^[\d|\w]*_(.*)$/, `${flow_id}$1`);
				}
				if(typeof(data[j].to_id) !== 'number' && data[j].to_id.indexOf('_') !== -1) {
	...
	arrObjDeepClone = (arr) => {
		let res = [];
		for(let i = 0, aLen = arr.length; i < aLen; i++) {
			let resObj = Object.assign({}, arr[i], {});
			res.push(resObj);
		}
		// console.log(res);
		return res
	 ...
	 * 数据中没有宽高，默认设置
	 */
	defaultNodeWH = (node, route) => {
		let f = this.fengCeng(node, route);
		let _lt = this.calc(f);
		// console.log(_lt);
		// let cloneNode = node.slice(0);
		let cloneNode = this.arrObjDeepClone(node);
		for (let k in _lt) {
			for(let j = 0, Jlen = _lt[k].length; j < Jlen; j++) {
				for(let i = 0, nLen = cloneNode.length; i < nLen; i++) {
					
					if(cloneNode[i].node_id == _lt[k][j].id) {
						cloneNode[i].point = {};
						cloneNode[i].point.left = _lt[k][j].left;
						cloneNode[i].point.top = _lt[k][j].top;
						cloneNode[i].point.width = 104;
						cloneNode[i].point.height = 26;
					}
				}
			}
	 ...
	 */
	calc = (obj) => {
		let center = document.getElementById('canvas_arrow').offsetWidth / 2;//
		let marTop = 10;//
		let marLeft = 20;
		let width = 104;
		let height = 26;
		...
		for (let key in obj) {
			fielObj[key] = [];
			let oLen = obj[key].length;
			for (let i = 0; i < oLen; i++) {
				let o = {};
				o.id = obj[key][i];
				o.top = key * (marTop + height);
				// if(oLen == 1) {
				// 	o.left = 500 - width / 2
				// }else (oLen % 2 == 0) {
				// 	let middle = oLen/2-1;
		...
		for (let k in fielObj) {
			let fLen = fielObj[k].length;
			if(fLen == 1) {
				fielObj[k][0].left = center - width / 2;
			}else if(fLen % 2 == 0) {
				let middle = fLen / 2 - 1;
				let p = middle,_stepMar = -114,_i=0 ;
				do {
						_stepMar = _stepMar - 124 * _i;
						fielObj[k][p].left =center + _stepMar;
						_i++;
						p--;
				}while(p > -1)
				let q = middle+1,_stepMarR = 10,_j = 0;
				do{
					_stepMarR = _stepMarR + 124 * _j;
					fielObj[k][q].left = center + _stepMarR
					_j++;
					q++;
				}while (q < fLen)
			}else if(fLen % 2 !== 0 && fLen !== 1) {
				let middle = parseInt(fLen / 2);
				let _p = middle, _q = middle + 1, _stepMar = -52, _stepMarR = 72, _i = 0,_j = 0;
				do{
					_stepMar = _stepMar - 124 * _i;
					fielObj[k][_p].left =center +  _stepMar
					_i++;
					_p--;
				}while(_p > -1)
				do {
					_stepMar = _stepMar + 124 * _j;
					fielObj[k][_q].left = center + _stepMarR
					_j++;
					_q++;
				}while (_q < fLen)
			}
	...
	/**
	 * 根据indexid获取第一层的nodeid
	 * @author fnd
	 * @param  {[type]} node [所有节点信息]
	 * @return {[type]}      [description]
	 */
	getFirstFuArr = (node) => {
	let firstFuArr = [];
		for(let i=0;i<node.length;i++){
			if(node[i].index_id == 1){
				firstFuArr.push(node[i].node_id);
			}
		}
		return firstFuArr;
	}
	 ...
	 * 去重函数
	 * @author fnd
	 * @param  {[type]} brr [description]
	 * @return {[type]}     [description]
	 */
	quChong = (brr) => {
		var arr = brr,result = [],i,j,len = arr.length;
		for(i = 0; i < len; i++){
			for(j = i + 1; j < len; j++){
				if(arr[i] === arr[j]){
					j = ++i;
	...
	/**
	 * 根据上一层和route以及所有已分层的数据找到下一层数据
	 * @author fnd
	 * @param  {[type]} nodeObj   [所有已经分层的数据]
	 * @param  {[type]} nodeFuArr [上一层数据]
	 * @param  {[type]} route     [description]
	 * @return {[type]}           [description]
	 */
	findChild = (nodeObj,nodeFuArr,route) => {
		let childArr = [];
		for(let i=0;i<nodeFuArr.length;i++){
			for(let j=0;j<route.length;j++){
				if(nodeFuArr[i]==route[j].from_id){
					childArr.push(route[j].to_id)
				}else if(nodeFuArr[i]==route[j].to_id){
					childArr.push(route[j].from_id)
				}
			}
		}
		childArr = this.quChong(childArr);
		// console.log(nodeObj);
		if(JSON.stringify(nodeObj) == "{}"){
			return childArr;
		}
		let newChildArr = [];
		for(let i=0;i<childArr.length;i++){
			let isChongFu = false;
			for(let j in nodeObj){//它可能连接到曾爷爷那里去，所以把该层的娃和祖先都对比下，去掉重复的
				for(let k=0;k<nodeObj[j].length;k++){
					if(childArr[i]==nodeObj[j][k]){
						isChongFu = true;
						// newChildArr.push(childArr[i])
					}
				}
				
			}
			if(!isChongFu){//该娃和祖先都不重复才push
				newChildArr.push(childArr[i]);
			}
	...
	fengCeng = (node,route) => {
		let nodeObj = {};
		let firstFuArr = this.getFirstFuArr(node);
		let k = 1;
		
		do{
			if(k==1){
				nodeObj[k] = firstFuArr
			}else if(k==2){
				nodeObj[k] = this.findChild({},nodeObj[k-1],route)
			}else{
				nodeObj[k] = this.findChild(nodeObj,nodeObj[k-1],route)
			}
			k++;
		}while(nodeObj[k-1].length!==0);
		if(nodeObj[k - 1].length == 0) delete nodeObj[k - 1]; 
		let nLen = node.length;
		let nodeArr = [];
		let nodeObjArr = [];
		for (let _key in nodeObj) {
			for (let _i = 0, _nLen = nodeObj[_key].length; _i < _nLen; _i++) {
				nodeObjArr.push(nodeObj[_key][_i]);
			}
		}
		for(let i = 0; i < nLen; i++) {
			nodeArr.push(node[i].node_id);
		}
		if(nodeObjArr.length !== nLen) {
			let buTong = this.twoArrFilter(nodeArr, nodeObjArr);
			for (let _i = 0, bLen = buTong.length; _i < bLen; _i++) {
				nodeObj[k] = [buTong[_i]];
				k++;
			}
		}
